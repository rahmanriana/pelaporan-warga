...
@section('content')
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
        <div>
            <h4 class="fw-bold mb-0">Daftar Laporan</h4>
            <div class="text-muted small">Menampilkan semua laporan.</div>
        </div>
        @auth
    ...
    <div class="row g-3">
        @forelse ($laporans as $laporan)
            @php
                ...
                $canEdit = auth()->check() && ((auth()->user()->role ?? null) === 'admin' || $laporan->user_id === auth()->id());
                ...
                <div class="card card-laporan h-100">
                        ...
                        <div class="d-flex justify-content-between align-items-start gap-2">
                            <div>
                                ...
                                <div class="text-muted small mt-1">
                                    <i class="fa-regular fa-calendar me-1"></i>{{ optional($laporan->created_at)->format('d M Y') }}
                                </div>
                    ...
                    <div class="card-footer bg-transparent border-0 pt-0 pb-3 px-3 d-flex gap-2">
                        ...
                        @if ($canEdit)
                            ...
                            <form method="POST" action="{{ route('laporans.destroy', $laporan) }}" class="d-inline" onsubmit="return confirm('Hapus laporan ini?')">
                                @csrf
                                @method('DELETE')
